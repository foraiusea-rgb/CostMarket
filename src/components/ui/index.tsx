...
export function MarketTypeBadge({ type }: { type: MarketType }) {
  ...
  return (
    <span className={`inline-flex items-center rounded px-1.5 py-0.5 text-[8px] font-semibold uppercase tracking-wider border ${s.bg} ${s.text} ${s.border}`}>
      {s.label}
    </span>
  );
}
...
export function ProviderBadge({ provider }: { provider: string }) {
  ...
  return (
    <span className="inline-flex items-center rounded px-1.5 py-0.5 text-[8px] font-semibold border"
...
export function Card({ children, className = "", hover = false, onClick }: {
  ...
  return (
    <div
      onClick={onClick}
      className={`rounded-[10px] border border-white/[0.06] bg-bg-2 ${
        hover ? "cursor-pointer transition-colors hover:border-white/[0.12] hover:bg-bg-3" : ""
...
export function StatCard({ label, value, color = "text-blue-400" }: { label: string; value: string | number; color?: string }) {
  return (
    <Card className="px-3 py-2.5">
      <div className="text-[9px] font-medium uppercase tracking-[0.06em] text-slate-500 mb-1">{label}</div>
